...
<body onload = "myalert()">
    <header>
      <div class="container">
        <div id="branding">
        <h1 style ="font-size:36; margin-bottom: 15px;"><span style ="font-size:36" class="highlight" st>Strategy</span> One</h1>
        </div>
    </header>
<?php
...
<div style = "margin-left:25px; margin-top:10px">
		...
		<a href="strategy2.php"><button type="button" class="btn btn-lg btn-link" style= "width:auto; float:right;margin-right:25px">Strategy Two</button></a>			
...
<?php
	if ($result->num_rows > 0)
    {
        while($row = $result->fetch_assoc()){
            echo "<tr><td>" . $row["size"] ."</td><td>" . $row["open_rate"] . "</td><td>" . $row["open_date"]."</td><td>" . $row["sl"] . "</td><td>" .$row["tp"]. "</td> <td>" . $row["close_rate"] . "</td> <td>" . $row["close_date"] . "</td><td>" . $row["P_L"] . "</td></tr>";
        }
    }      
        else{ echo "0 reults";
	}
