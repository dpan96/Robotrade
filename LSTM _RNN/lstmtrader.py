...
def train(trader, features, labels, train_steps=3700, batch_size=32, validation_size=600):
    initial_step = 1
        ...
        for i in range(initial_step, initial_step + train_steps):
            ...
            if i % VERBOSE_STEP == 0:
                ...
                if i % VALIDATION_STEP == 0:
                    ...
                    sno_list.append(i)
                    avg_train_loss_list.append(loss)
                    avg_pos_list.append(avg_pos)
                    val_loss_list.append(val_loss)
                    val_avg_pos_list.append(val_avg_pos)
                    ...
                    avg_train_loss_list.append(loss)
                    avg_pos_list.append(avg_pos)
                    val_loss_list.append("NaN")
                    val_avg_pos_list.append("NaN")
                print(hint)
                
    df = pd.DataFrame(data={"Steps": sno_list, "Average Train Loss":avg_train_loss_list, "Average position": avg_pos_list, "Validation Loss": val_loss_list, "Validation Average Position": val_avg_pos_list})
    df.to_csv("./train_csv_file_M15.csv", sep=',',index=False)
...
def predict(features, labels):
    ...
    hidden_size = 8
    nclasses = 1
    trader = Robotrade(step, input_size, learning_rate, hidden_size, nclasses)
    trader.build_graph()
    saver = tf.train.Saver()
    label_list= []
    pred_list= []
    with tf.Session() as sess:
        ...
        for i in range(len(labels)):
            ...
            label_list.append(str(labels[i][0]))
            pred_list.append(str(pred[i][0]))
    df = pd.DataFrame(data={"ChangeRate": label_list, "PositionAdvice": pred_list})
   

def main(operation = 'train', dataset = 'GBP_USD_D.csv'):
    step = 30
    input_size = 47
    train_step = 3700
    batch_size = 32
    ...
    selector = ["ROCP", "OROCP", "HROCP", "LROCP", "MACD", "RSI", "UO", "BOLL", "MA", "STOCH", "AO"]
    input_shape = [30, 47]  # [length of time series, length of feature]
    raw_data_io = rawdata.read_sample_data(dataset)
    moving_features, moving_labels = extract_feature(raw_data=raw_data_io, selector=selector, window=input_shape[0],
    ...
    if operation == 'train':
        trader = Robotrade(step, input_size, learning_rate, hidden_size, nclasses)
        trader.build_graph()
        train(trader, moving_features, moving_labels, train_step, batch_size)
    else:
        df = predict(moving_features, moving_labels)
        
        df.to_csv("./pred_csv_file_GU_D.csv", sep=',',index=False)

if __name__ == '__main__':
    dataset = 'GBP_USD_D.csv'
    operation = 'train'
    if len(sys.argv) > 1:
        dataset = sys.argv[2]
