...
def run_all(predict_pnl=True, print_explained_trades=True, return_dataframe=False, threshold = 0.0020):
    ...
    for index, row in df.iterrows():
        ...
        if index > 59:
            ...
            cols_to_norm = ["ROCP1", "ROCP2", "OROCP", "HROCP", "LROCP", "norm_macd", "norm_signal", "norm_hist", "macdrocp", "singalrocp", "histrocp",
                "rsi6", "rsi12", "rsi24", "rsi6rocp", "rsi12rocp", "rsi24rocp", "UO", "upperBOLL", "middleBOLL",
                "lowerBOLL", "MA5rocp", "MA10rocp", "MA20rocp", "MA25rocp", "MA30rocp", "MA40rocp", "MA50rocp", "MA60rocp", 
                "MA5", "MA10", "MA20", "MA25", "MA30", "MA40", "MA50", "MA60", "Slow_stochk", "Slow_stochd", 
                "Fast_stochk", "Fast_stochd", "Fast_stoch_rsik", "Fast_stoch_rsid", "AO","ROC5", "ROC10", "ROC20",
            ...
            df_X = mm_scaler.fit_transform(df[cols_to_norm])
            #df_X = select_k_best.transform(df_X[60:])
            #df_X = df_X[:, 49]
    
            dim = int(np.sqrt(len(cols_to_norm)))
            df_X = reshape_as_image(df_X, dim, dim)
            # adding a 1-dim for channels (3)
            df_X = np.stack((df_X,) * 3, axis=-1)
            
            #df_X = df_X.iloc[:, 0:].values
...
def main(operation = 'run_for', currency_pair = 'AUD/USD', duration = 'H4', past_rows = 1000):
    ...
    '''
    select_k_best = SelectKBest(mutual_info_classif, k=53)
...
if __name__ == '__main__':
    ...
    """cd "Documents\Capstone Project\Code_backup\CNN_7x7_image_processing_not_showing_results"
        python CNN_live_backtest_row_by_row_input.py run_for AUD/USD H4 1000
