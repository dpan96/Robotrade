...
def run_all(df, predict_pnl=True, print_explained_trades=True, return_dataframe=False, threshold = 0.0020, classifier=True):
    ...
    col_names =  ['size', 'open_rate', 'open_date','sl','tp','close_rate','close_date','P_L']
            ...
            if np.isnan(row['close_rate']) and ~np.isnan(row['open_rate']):
                if (price - df['open_rate'][index]) < tp and (price - df['open_rate'][index]) > sl:
                    if signal == 'Buy' or signal == 'Sell':
                        ...
                        df['P_L'][index] =  df['close_rate'][index] - df['open_rate'][index]
                        print("profit/loss")
    ...
    for index, row in df.iterrows():
        ...
        if signal != "":
            if signal == "Buy":
                sy1 = sys1(sy1,signal,row['open'],row['datetime'],sl,tp)
            if signal == "Stay":
    ...
    for index, row in sy1.iterrows():
        ...
        if np.isnan(row['P_L']):
            ...
            profit += row['P_L'] * 10000
            if row['P_L'] > 0:
                positive += 1
            else:
                negative += 1
    print("Total amount of trade made: ", total, ", and", count, "trades not closed")        
    print("There are", positive, "winning trades and", negative, "lossing trades.", "Based on current closed trades, the P_L is : ", profit)
...
def create_table(username, password, dbname, currency_pair, duration): #DON'T PUT PASSWORD,ETC HERE; this is a function
    ...
    print(currency_pair)
    sqlQuery            = "CREATE TABLE " + currency_pair + duration + " (size varchar(30), open_rate varchar(50), open_date varchar(50), sl float, tp float, close_rate varchar(50), close_date varchar(50), P_L varchar(50))"
...
def store_in_sql(results,username, password, dbname, currency_pair, duration):#DON'T PUT PASSWORD,ETC HERE; this is a function
    ...
    for ind,row in results.iterrows():
        try:
            sqlQuery            = "INSERT INTO "  + currency_pair + duration + " (size, open_rate , open_date , sl , tp, close_rate, close_date, P_L) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
            inserting_vals = (str(row['size']), str(row['open_rate']), str(row['open_date']), float(row['sl']), float(row['tp']), str(row['close_rate']), str(row['close_date']), str(row['P_L']))    
            ## Inserting values... 
...
def main(operation = 'run_for', currency_pair = 'all', duration = 'all', past_rows = 200, dbname = 'final1', username = 'root', password = ''):
...
if __name__ == '__main__':
    ...
    """cd "Documents\Robotrade\Keras_NN_with_softmax_relu"
        python Keras_NN_live_backtest_row_by_row_input_php.py run_for AUD/USD H1 200 dbname: final1 username: root password:
        python Keras_NN_live_backtest_row_by_row_input_php.py run_for all all 200 dbname: final1 username: root password:
